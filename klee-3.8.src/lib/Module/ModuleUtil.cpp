...
static bool linkBCA(object::Archive* archive, Module* composite, std::string& errorMessage)
{
       ...
       AE = archive->child_end(); AI != AE; ++AI)
  {
    ...
    std::error_code ec = AI->get().getName().getError();
    ...
    bool success = (bool) AI->get().getAsBinary();
    if (!success)
    {
      ...
      if (std::error_code ec = AI->get().getBuffer().getError())
      {
      ...
      if (buff)
      {
        ...
        ErrorOr<std::unique_ptr<Module>> parse_err = parseBitcodeFile(*buff.get(), getGlobalContext());

        if(!parse_err)
        {
          SS << "Loading module failed : " << /* parse_err.getError() *<< */ "\n";
          SS.flush();
          return false;
        }
    ...
    for (size_t i = 0, j = archiveModules.size(); i < j; ++i)
    {
         ...
         S != SE; ++S)
      {
        ...
        if ( GlobalValue* GV = dyn_cast_or_null<GlobalValue>(M->getValueSymbolTable().lookup(*S)))
        {
          ...
          std::unique_ptr<Module> mptr(M);
          if (Linker::linkModules(*composite, std::move(mptr), Linker::Flags::OverrideFromSrc))
          {
            // Linking failed
...
Module *klee::linkWithLibrary(Module *module, 
  ...
  if (magic == sys::fs::file_magic::bitcode) {
    //Module *Result = 0;
    ErrorOr<std::unique_ptr<Module> > Result = parseBitcodeFile(*Buffer->get(), Context);


    if (!Result || Linker::linkModules(*module, std::move(*Result), Linker::Flags::OverrideFromSrc))
      klee_error("Link with library %s failed: %s", libraryName.c_str(),
        /* ErrorMessage.c_str() */ "???");
...
//    delete Result;
    ...
    if (ErrorOr<std::unique_ptr<object::Binary> > ec = object::createBinary(*Buffer->get(), &Context))
      klee_error("Link with library %s failed: %s", libraryName.c_str(),
          /* ec.message().c_str() */ "???");
