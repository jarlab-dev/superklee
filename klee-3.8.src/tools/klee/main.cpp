...
int main(int argc, char **argv, char **envp) {
  ...
  std::unique_ptr<MemoryBuffer> bptr(Buffer->get()); auto mainModuleOrError = getLazyBitcodeModule(std::move(bptr), getGlobalContext());
  ...
  mainModule = mainModuleOrError->get();
  if (auto ec = mainModule->materializeAll()) {
    klee_error("error loading program '%s': %s", InputFile.c_str(),
               ec.message().c_str());
  }
  (*mainModuleOrError).release();
#endif
