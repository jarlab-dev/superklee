...
//  executionEngine = ExecutionEngine::createJIT(dispatchModule, &error);
  std::unique_ptr<Module> MPtr(dispatchModule);
  executionEngine = EngineBuilder(std::move(MPtr)).setEngineKind(EngineKind::JIT).create();
...
//    sys::DynamicLibrary::LoadLibraryPermanently("/usr/lib/x86_64-linux-gnu/libc.so");
    
//    "/usr/lib/x86_64-linux-gnu/libc.so"
//    executionEngine->addObjectFile();
  }
...
//      executionEngine->recompileAndRelinkFunction(dispatcher);
      dispatcher->setLinkage(Function::AvailableExternallyLinkage);  
      std::map<std::string, void*>::iterator it2 = 
        preboundFunctions.find(f->getName());
      ...
      if (it2 != preboundFunctions.end()) {
        // only bind once
        if (it2->second) {
          executionEngine->updateGlobalMapping(dispatcher->getName(), (uint64_t) it2->second);
...
Function *ExternalDispatcher::createDispatcher(Function *target, Instruction *inst) {
  ...
  Function *dispatcher = Function::Create(FunctionType::get(Type::getVoidTy(getGlobalContext()), 
					  ...
					  "",
					  dispatchModule);
  dispatcher->setName(target->getName());
  ...
  for (CallSite::arg_iterator ai = cs.arg_begin(), ae = cs.arg_end();
    ...
    LLVM_TYPE_Q Type *argTy = (i < FTy->getNumParams() ? FTy->getParamType(i) : 
                               (*ai)->getType());
    Instruction *argI64p = 
      GetElementPtrInst::Create(NULL, argI64s, 
